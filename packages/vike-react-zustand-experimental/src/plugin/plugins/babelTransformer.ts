...
type TransformResult = {
  code: string
  map: any
  hasVikeReactZustandExperimental: boolean
...
export async function transformCode(code: string, options: TransformOptions): Promise<TransformResult> {
  try {
    ...
    const state: State = {
      modified: false,
      hasVikeReactZustandExperimental: false,
    ...
    if (!result || !state.hasVikeReactZustandExperimental || !state.modified) {
      return {
        code,
        map: null,
        hasVikeReactZustandExperimental: state.hasVikeReactZustandExperimental,
        storeKeys: new Set(),
      }
    }
  ...
  return {
    visitor: {
      ImportDeclaration(path) {
        if (path.node.source.value !== 'vike-react-zustand-experimental') {
          return
        }
        state.hasVikeReactZustandExperimental = true
 ...
 * Plugin to strip transfer() calls
 */
function stripTransferPlugin(state: State): PluginItem {
  return {
    visitor: {
      CallExpression(path) {
        if (!t.isIdentifier(path.node.callee) || !state.transferNames.has(path.node.callee.name)) {
          return
        }
